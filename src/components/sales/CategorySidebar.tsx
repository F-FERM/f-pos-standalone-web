...
export function CategorySidebar({
  ...
  const categoriesQuery = useQuery({
    ...
    queryFn: () => ListCategoryApi({ page: 1, limit: 100 }),
  });
  ...
  const truncateLabel = (name: string, max = MAX_LABEL_LENGTH) =>
  ...
  return (
    ...
    >
      ...
      <div
        ref={scrollRef}
        className="
          flex h-full min-h-0 w-full flex-col items-center gap-3 overflow-y-auto overflow-x-hidden py-3
        ...
        {categories.map((category) => {
          ...
          return (
            <div
              ...
              className="relative flex w-full min-w-0 max-w-full shrink-0 items-center justify-center "
            >
              {selected ? (
                <button
                  type="button"
                ...
                <button
                  ...
                  onClick={() => onSelect(category._id)}
                  title={category.name}
                  className="relative z-10 flex w-full min-w-0 max-w-full flex-col items-center justify-center overflow-hidden px-1 py-3"
                >
                  <Icon
                  ...
                  <span
                    title={category.name}
                    className="
                      mt-0.5 block w-full min-w-0 max-w-full overflow-hidden
                      whitespace-nowrap text-center
                      text-[10px] font-medium leading-tight text-[#3B0038]
                      xs:text-[12px] md:text-[12px] md:leading-[14px]
                    "
                  >
